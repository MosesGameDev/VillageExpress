...
public class AIVehicleController : PositioningAIControl
{
    ...
    private void ForwardMove()
    {
        ...
        if (AheadRB)
        {
            ...
            float aheadRBSpeed = Mathf.Lerp(AheadRB.linearVelocity.magnitude, desiredSpeed, Aggressiveness);
    ...
    void UpdateAdditionalHits()
    {
        ...
        {
            Physics.Raycast(transform.TransformPoint(AdditionalHitPoints[i]), transform.forward, out AdditionalHits[i], ObstacleHitDistance + Car.Bounds.size.z, AiDetectionMask);
        }
    }
